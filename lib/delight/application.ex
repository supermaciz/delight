defmodule Delight.Application do
  # See https://elixir.hexdocs.pm/Application.html
  # for more information on OTP Applications
  @moduledoc false

  use Application

  @impl true
  def start(_type, _args) do
    Delight.HTTPLogger.attach()

    children = [
      DelightWeb.Telemetry,
      Delight.Repo,
      {DNSCluster, query: Application.get_env(:delight, :dns_cluster_query) || :ignore},
      {Phoenix.PubSub, name: Delight.PubSub},
      # Start a worker by calling: Delight.Worker.start_link(arg)
      # {Delight.Worker, arg},
      {Delight.DeezerAPI.RateLimiter, clean_period: :timer.minutes(1)},
      # Start to serve requests, typically the last entry
      DelightWeb.Endpoint
    ]

    # See https://elixir.hexdocs.pm/Supervisor.html
    # for other strategies and supported options
    opts = [strategy: :one_for_one, name: Delight.Supervisor]
    Supervisor.start_link(children, opts)
  end

  # Tell Phoenix to update the endpoint configuration
  # whenever the application is updated.
  @impl true
  def config_change(changed, _new, removed) do
    DelightWeb.Endpoint.config_change(changed, removed)
    :ok
  end
end
